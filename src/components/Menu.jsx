  ...
  return (
    <>
      <button
        ref={toggleButtonRef}
        onClick={toggleMenu}
        className="z-50 absolute top-2 right-4 p-3 bg-white border w-11 h-11 rounded-md"
      >
        <div
      ...
      <div
        ref={menuRef}
        className={`z-40 absolute top-0 right-0 bottom- bg-white text-black transition-all overflow-hidden flex flex-col
        ${menuOpened ? "w-80 h-[100vh]" : "w-0 h-[100vh]"}`}
      >
        <div className="flex-1 flex items-center justify-start mt-16 text-lg font-semibold flex-col gap-6 p-8">
          <>
